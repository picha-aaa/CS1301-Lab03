...
if "neo_df" not in st.session_state:
    st.session_state["neo_df"] = None
if "neo_start_date" not in st.session_state:
    st.session_state["neo_start_date"] = None
if "neo_end_date" not in st.session_state:
    st.session_state["neo_end_date"] = None
...
if st.button("Fetch Asteroids 🚀"):
    with st.spinner("Pulling data from NASA NeoWs..."):
        try:
    ...
    if df is not None and not df.empty:
        ...
        st.session_state["neo_start_date"] = start_date
        st.session_state["neo_end_date"] = end_date

        st.success(f"Loaded {len(df)} asteroid records and stored them for other pages.")
